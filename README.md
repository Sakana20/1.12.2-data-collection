...
推荐使用本地服务模式，它会实时读取录制文件和 JourneyMap 瓦片，只加载当前地图视口需要的图片。连接后可以在页面顶部切换 `NebulaeCraft` 与 `NebulaeCraft DH` 两个录制数据源；地图瓦片默认固定读取 `D:\Minecraft\NebulaeCraft\.minecraft`：

```powershell
powershell -ExecutionPolicy Bypass -File tools\visualizer\visualizer-server.ps1
```
...
```text
数据源 1: D:\Minecraft\NebulaeCraft\.minecraft\nebulaestats
数据源 2: D:\Minecraft\NebulaeCraft_DH\.minecraft\nebulaestats
地图源:   D:\Minecraft\NebulaeCraft\.minecraft\journeymap\data\mp\NebulaeCraft\DIM0
```

如果 JourneyMap 地图源或世界目录不同，可以传参数：

```powershell
powershell -ExecutionPolicy Bypass -File tools\visualizer\visualizer-server.ps1 -MapMinecraftDir "D:\Minecraft\NebulaeCraft\.minecraft" -JourneyMapWorld "mp\NebulaeCraft\DIM0"
```
